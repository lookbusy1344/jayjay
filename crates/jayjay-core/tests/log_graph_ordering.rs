...
#[test]
fn log_graph_defaults_to_present_at_prioritization_without_config() {
    let temp_dir = init_jj_repo();
    let repo_path = temp_dir.path().join("repo");
    let repo_str = repo_path.to_str().expect("repo path utf-8");

    // Two independent heads off the base change; @ stays on the older, lower-index one.
    // Only present(@) prioritization floats @'s head above the newer sibling; a bare topological
    // grouping emits the higher-index head first.
    run_jj_in(&repo_path, &["describe", "-m", "base"]);
    run_jj_in(
        &repo_path,
        &["new", "subject(exact:\"base\")", "-m", "older"],
    );
    run_jj_in(
        &repo_path,
        &["new", "subject(exact:\"base\")", "-m", "newer"],
    );
    run_jj_in(&repo_path, &["edit", "subject(exact:\"older\")"]);

    // Clear the repo-config pin so the in-process side must fall back to the CLI's built-in present(@).
    run_jj_in(
        &repo_path,
        &["config", "unset", "--repo", "revsets.log-graph-prioritize"],
    );
    let repo = Repo::open(&repo_path).expect("open repo");
    ...
    let older = cli_commit_for(repo_str, "older").expect("older present");
    let newer = cli_commit_for(repo_str, "newer").expect("newer present");
    let older_index = ours
        .iter()
        .position(|id| *id == older)
        .expect("older in graph");
    let newer_index = ours
        .iter()
        .position(|id| *id == newer)
        .expect("newer in graph");
    assert!(
        older_index < newer_index,
        "working-copy head should precede the newer sibling via the present(@) default"
    );
}
