...
#[test]
fn exhausting_the_revset_at_the_row_ceiling_finishes_without_pausing() {
    let (_temp_dir, repo_path) = build_linear_repo(7);
    let repo = Repo::open(&repo_path).expect("open repo");
    let full = repo.log_graph("all()").expect("full load");
    let mut bounded = request("all()", full.len() as u32, 3);
    bounded.row_ceiling = full.len() as u32;
    let mut events = Vec::new();

    repo.start_log_graph(bounded, GraphLoadToken::new(), |event| {
        events.push(event);
    });
