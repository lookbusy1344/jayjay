...
use crate::dag::{DagLayout, DagLayoutInput};
use crate::types::*;
...
/// `jj log`'s built-in default for `revsets.log-graph-prioritize`. The value lives in the CLI, not
/// in `jj-lib` settings, so the in-process graph path must supply it to match `jj log`'s ordering.
const DEFAULT_LOG_GRAPH_PRIORITIZE: &str = "present(@)";
...
impl Repo {
    ...
    /// `ui.log-synthetic-elided-nodes` for the current repository settings, for callers (UniFFI
    /// included) computing a `DagLayout` outside a progressive session, which already resolves
    /// this itself.
    pub fn log_graph_synthetic_elided_nodes(&self) -> CoreResult<bool> {
        let repo = self.get_repo();
        self.synthetic_elided_nodes_enabled(&repo)
    }
    ...
    fn run_log_graph_session(
        ...
        let synthetic_elided_nodes = self.synthetic_elided_nodes_enabled(&repo)?;
        let root_commit_id = repo.store().root_commit_id();
        let mut stream = std::pin::pin!(topo_order.stream());
        ...
        loop {
            ...
            // Only pause after observing another real row; reaching the ceiling exactly as the stream ends is a complete result, not a request for the user to continue loading.
            ...
            let available_to_publish = raw_rows.len() as u32;
    ...
    /// `ui.log-synthetic-elided-nodes`, matching `jj log`'s own default of `true` when unset.
    fn synthetic_elided_nodes_enabled(&self, repo: &Arc<ReadonlyRepo>) -> CoreResult<bool> {
        repo.settings()
            .get_bool(["ui", "log-synthetic-elided-nodes"])
            .optional()
            .map_err(|e| CoreError::Internal {
                message: format!("read ui.log-synthetic-elided-nodes: {e}"),
            })
    ...
    /// snapshot's cumulative entries are cloned from that buffer. The layout is computed directly
    /// from the same real-row prefix, matching `jj log --limit N` without materializing full
    /// `ChangeInfo` values first.
    #[allow(clippy::too_many_arguments)]
    fn publish_log_graph_prefix(
        ...
        let publish_len = (threshold as usize).min(raw_rows.len());
        ...
        let published_ids = raw_rows[..publish_len]
            .iter()
            .map(|(commit, _)| commit.id().hex())
            .collect::<HashSet<_>>();
        let refs = CommitRefIndex::build(repo, self.workspace_name.as_ref(), &published_ids);
        // Exactly the published prefix, never a row further down the stream: `jj log --limit N`
        // renders N rows and nothing behind them, and a row the shell cannot see must not steer
        // the renderer state the rows it can see are drawn from.
        let layout_inputs = raw_rows[..publish_len]
            .iter()
            .map(|(commit, edges)| raw_layout_input(commit, edges, &refs))
            .collect::<Vec<_>>();
        let layout = DagLayout::compute_inputs(&layout_inputs, synthetic_elided_nodes);
        if guard.is_canceled() {
            return Ok(false);
        }
    ...
    /// Commit IDs matching `revsets.log-graph-prioritize`, intersected with `expression`, in the order the config revset yields them. Falls back to `jj log`'s built-in `present(@)` default when the key is unset.
    fn log_graph_prioritized_ids(
        ...
        let configured = repo
            ...
            .unwrap_or_default();
        let prioritize_revset_str = match configured.trim() {
            "" => DEFAULT_LOG_GRAPH_PRIORITIZE,
            trimmed => trimmed,
        };
