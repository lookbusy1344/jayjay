...
final class RepoViewModel: ChangeActions, DAGActions, BookmarkActions {
    ...
    #if DEBUG
        /// Test seam: seed graph entries directly, matching a bare assignment (no layout or generation
        /// side effects), for tests that only need rows present.
        func seedGraphEntriesForTesting(_ entries: [GraphEntry]) {
            graphEntries = entries
        }
    #endif
