...
extension RepoViewModel {
    ...
    func applyGraphSnapshot(
        ...
        // Keep a selection the user made mid-refresh: it differs from the baseline captured at refresh
        // start, so the loaded change must not override it.
        guard selectionBaseline == nil || selectionBaseline == selectedChangeIds else {
            graphPendingSelectedChange = nil
            return
        }
