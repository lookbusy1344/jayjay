...
use super::dag::{DagGeometry, OVERFLOW_BADGE_TAP_SIZE, dag_column, overflow_badge_center_x};
use super::dag_drag::DagDragSelection;
use super::dag_row::{
    ChipRightClick, DagDrop, DagRow, dag_row, node_center_offset, row_background,
};
            ...
            cx.processor(move |this, range: std::ops::Range<usize>, _window, cx| {
                ...
                range
                    .map(|ix| {
                        ...
                        let row_bg =
                            row_background(&t, is_selected, is_compare_source, is_pane_active);
                        ...
                        let graph_width = dag_layout.rows.get(ix).map_or(0., |row| {
                            dag_geometry.row_graph_width(row.graph_column_count).width
                        });
                        let refs_budget = width - graph_width - t.scaled_font_size(12.);
                        dag_row(
                            DagRow {
...
fn focus_badge_overlay(
    ...
    let revision: SharedString = change.selection_revision().into();
    ...
    div()
        ...
        .child(
            div()
                .id(id)
                .absolute()
                .top(px(node_center_offset(theme) - OVERFLOW_BADGE_TAP_SIZE / 2.0))
